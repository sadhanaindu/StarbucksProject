...
public class StarbucksService {
    ...
    public StarbucksOrder newOrder(String regid, StarbucksOrder order) throws ResponseStatusException {
        ...
        List<StarbucksOrder> starbOrders = ordersRepository.findByRegister(regid);
        boolean active = false;

        for(StarbucksOrder o: starbOrders)
        {
            if(o.getStatus().equals("Ready for Payment."))
            {
                active = true;
            }
        }
        //StarbucksOrder active = starbOrders.get(regid);
        if (active) {
        ...
        rabbit.convertAndSend(queue.getName(), new_order.getId());
        return new_order;
    }
    ...
    public StarbucksOrder getActiveOrder(String regid) {
        boolean active = false;
        StarbucksOrder order = null;
        List<StarbucksOrder> starbOrders = ordersRepository.findByRegister(regid);

        for(StarbucksOrder o: starbOrders)
        {
            if(!(o.getStatus().equals("paid") || o.getStatus().equals("FULFILLED")))
            {
                active = true;
                order = o;
            }
    ...
    public void clearActiveOrder(String regid) {
        ...
        for(StarbucksOrder o: starbOrders)
        {
            if(!(o.getStatus().equals("paid") || o.getStatus().equals("FULFILLED")))
            {
                ...
                //ordersRepository.deleteById(o.getId());
    ...
    /* RabbitMQ */
    public StarbucksOrder getOrderById(String id)
    {
        long number = Long.parseLong(id);
        Optional<StarbucksOrder> order = ordersRepository.findById(number);
        if(order.isPresent()) {
            StarbucksOrder list = order.get();
            list.setStatus("FULFILLED");
